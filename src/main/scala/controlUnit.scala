...
class Controller(memoryDepth: Int, memoryHeight: Int, datawidth: Int, numberOfPE: Int) extends Module{
    val io = IO(new Bundle{
        val controller_in = new ControllerInterfaceIn(memoryDepth, memoryHeight, datawidth, numberOfPE)
        val controller_out = new ControllerInterfaceOut(memoryDepth, memoryHeight, datawidth, numberOfPE)
    })
...
object DriverController extends App{
    (new chisel3.stage.ChiselStage).emitVerilog(new Controller(8, 32, 16, 4), Array("--target-dir", "generated/"))
}
