...
class NeuronArchitectureInterfaceOut(memoryDepth: Int, memoryHeight: Int, datawidth: Int, numberOfPE: Int, tableDepth: Int) extends Bundle{
    ...
    val PE_outputs = Output(Vec(numberOfPE, UInt(datawidth.W)))
...
//  Neuron Inputs
    ...
    io.neuron_architecture_out.neuron_datainput := datapoint_memory.io.rdData    
    
    when(control_unit.io.controlUnit_out.read_memoryUnits(0) === 1.U(2.W)){
    	io.neuron_architecture_out.neuron_weights(0) := weight_memory_1.io.rdData
    } .elsewhen(control_unit.io.controlUnit_out.read_memoryUnits(0) === 2.U(2.W)){
    	io.neuron_architecture_out.neuron_weights(0) := weight_memory_2.io.rdData
    } .otherwise{
    	io.neuron_architecture_out.neuron_weights(0) := 0.U(datawidth.W)
    }

    when(control_unit.io.controlUnit_out.read_memoryUnits(1) === 1.U(2.W)){
    	io.neuron_architecture_out.neuron_weights(1) := weight_memory_3.io.rdData
    } .elsewhen(control_unit.io.controlUnit_out.read_memoryUnits(1) === 2.U(2.W)){
    	io.neuron_architecture_out.neuron_weights(1) := weight_memory_4.io.rdData
    } .otherwise{
    	io.neuron_architecture_out.neuron_weights(1) := 0.U(datawidth.W)
    }

    when(control_unit.io.controlUnit_out.read_memoryUnits(2) === 1.U(2.W)){
    	io.neuron_architecture_out.neuron_weights(2) := weight_memory_5.io.rdData
    } .elsewhen(control_unit.io.controlUnit_out.read_memoryUnits(2) === 2.U(2.W)){
    	io.neuron_architecture_out.neuron_weights(2) := weight_memory_6.io.rdData
    } .otherwise{
    	io.neuron_architecture_out.neuron_weights(2) := 0.U(datawidth.W)
    }

    when(control_unit.io.controlUnit_out.read_memoryUnits(3) === 1.U(2.W)){
    	io.neuron_architecture_out.neuron_weights(3) := weight_memory_7.io.rdData
    } .elsewhen(control_unit.io.controlUnit_out.read_memoryUnits(3) === 2.U(2.W)){
    	io.neuron_architecture_out.neuron_weights(3) := weight_memory_8.io.rdData
    } .otherwise{
    	io.neuron_architecture_out.neuron_weights(3) := 0.U(datawidth.W)
    }
    ...
    io.neuron_architecture_out.controlUnit_current_state := control_unit.io.controlUnit_out.controlUnit_current_state
    io.neuron_architecture_out.load_pe_memory_load_datapoint_state := control_unit.io.controlUnit_out.load_pe_memory_load_datapoint_state
    io.neuron_architecture_out.load_pe_memory_load_weights_state := control_unit.io.controlUnit_out.load_pe_memory_load_weights_state

    io.neuron_architecture_out.PE_outputs(0) := neuron_1.io.neuron_out.asUInt
    io.neuron_architecture_out.PE_outputs(1) := neuron_2.io.neuron_out.asUInt
    io.neuron_architecture_out.PE_outputs(2) := neuron_3.io.neuron_out.asUInt
    io.neuron_architecture_out.PE_outputs(3) := neuron_4.io.neuron_out.asUInt
